...
2. At the very top of the Overview blade, click **Download publish profile**.
   - **Note:** If you see an error that "Basic Authentication is disabled", you must temporarily enable it. Go to **Configuration** -> **General settings** -> **Basic Auth Publishing Credentials** and set the **SCM** option to **On**. Save the change, then go back and download the profile.
